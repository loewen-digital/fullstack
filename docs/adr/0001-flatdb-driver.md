...
What flatdb offers (0.2.1 on npm):
...
- Tests run against flatdb's real `MemoryAdapter` and collections: a structural contract is worth testing against the real implementation. flatdb 0.2.1 is on npm and becomes a devDependency from there; #3 adds it.
